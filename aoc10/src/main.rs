...
fn part_1(instructions: &Vec<Option<i64>>) -> i64 {
    ...
    let mut queue: VecDeque<Option<i64>> = instructions.iter()
        .flat_map(|&i| if i.is_some() {vec![None, i]} else {vec![None]} ).collect();
    let mut x = 1;
    let mut signal = 0;
    for cycle in 1..=220 {
...
fn part_2(instructions: &Vec<Option<i64>>) -> String {
    let mut queue: VecDeque<Option<i64>> = instructions.iter().flat_map(|i| match *i {
        None => vec![None],
        v => vec![None, v],
    }).collect();
    let mut x = 1;
    ...
    for cycle in 0..240 {
        ...
        display.push(if (cycle as i64 % 40).abs_diff(x) <= 1 { '#' } else { ' ' });
        if let Some(Some(v)) = queue.pop_front() {
            x += v;
        }
    }
